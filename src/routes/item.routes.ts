...
router.put("/:id", async (req, res) => {
  try {
    ...
    const result = await itemService.updateOne(parseInt(id), req.body);
...
router.get("/:id", async (req, res) => {
  try {
    ...
    //         .limit(1);
    const result = await itemService.findById(parseInt(id));
    return res.status(201).json({
...
router.delete("/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await itemService.deleteOne(parseInt(id));
    return res.status(201).json({
